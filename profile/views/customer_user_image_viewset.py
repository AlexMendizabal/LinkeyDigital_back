...
from profile.models import CustomerUserCustomImage , CustomImageDto
from profile.services import ContactService


class CustomerUserCustomImageSerializer(serializers.ModelSerializer):
    class Meta:
...
class CustomerUserImageViewSet(APIView):
    def get(self, request, pk=None):
        try:
            social_image = ContactService()
            try:
                response = social_image.get_custom_image(pk, request.user.id)
            except Exception as e:
                return Response({"succes": False}, status=status.HTTP_404_NOT_FOUND)

            customer_custom_image_serializers = CustomerUserCustomImageSerializer(response, many=True)
    ...
    def post(self, request):
        ...
        serializer = CustomerUserCustomImageSerializer(data=request.data)

        if not serializer.is_valid():
            return Response({"status": "error", "data": serializer.errors}, status=status.HTTP_400_BAD_REQUEST)

        dto = self.build_dto_from_validated_data(serializer)
        social_image = ContactService()
        ...
        customer_custom_image_serializers = CustomerUserCustomImageSerializer(response, many=False)
        return Response({"success": True, "data": customer_custom_image_serializers.data},
                        status=status.HTTP_200_OK)
    ...
    def put(self, request, pk=None):
        ...
        customer_user_custom_social_media_serializers = CustomerUserCustomImageSerializer(
    ...
    def build_dto_from_validated_data(self, serializer):
