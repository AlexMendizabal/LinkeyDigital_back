...
from profile.services import ViewsServices
...
class ViewsViewSet(APIView):
    def get(self, request, profile=None, month=None, year=None):
        try:
            ...
            view_service = ViewsServices()
            try:
                response = view_service.get_views(profile, month,year)
            except Exception as e:
                print(e)
                return Response({"succes": False}, status=status.HTTP_404_NOT_FOUND)

            viewsSerealized = ViewProfileSerializer(response, many=True)
