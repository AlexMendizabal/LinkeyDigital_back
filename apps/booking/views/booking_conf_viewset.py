...
from apps.booking.models import ConfigurationBooking,ConfigurationBookingDto
...
class ConfBookingViewset(APIView):
    ...
    def post(self, request, user_id=None):
        ...
        booking_serializers = ConfBookingserializers(data=request.data)
        ...
        try:
            response = booking_services.create_or_update_conf_booking(dto)
        except Exception as e:
            return Response({"success": False, "error" : e}, status=status.HTTP_503_services_UNAVAILABLE)
        customer_email_serializers = ConfBookingserializers(response, many=False)
