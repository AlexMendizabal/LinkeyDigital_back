...
from apps.administration.views import Licenciaserializers, Utilities

#WAITING: poner los serializadores en sus archivos o lugares respectivos

class CustomerUserserializersr(serializers.ModelSerializer):
...
class CustomerUserProfileserializersLow(serializers.ModelSerializer):
    customer_user = CustomerUserserializersr()
    class Meta:
...
class Profileservices:
    ...
    def get_all_profile_licencia(self, licencia_id, id_admin = None):
        ...
        for customer_user in customer_users:
            ...
            customer_profile_serializers = CustomerUserProfileserializersLow(profile, many=False)
            user_serializers = CustomerUserserializers(customer_user, many=False)
    ...
    #     latests_ids = Licencia.objects.values('customer_user_admin').annotate(max_id=Max('id'))
    #     # Obtener las licencias correspondientes a los customer_user_id más recientes
    #     licencias = Licencia.objects.filter(id__in=latests_ids.values('max_id'))
    #     response = []
    ...
    #             licencia_serializ = Licenciaserializers(licencia, many=False)
    #             utilities = Utilities()
    ...
    #             custom_user_serializ = CustomerUserserializers(custom_user, many=False)

    #             profile_user_serializ = CustomerUserProfileserializersLow(profile_user, many=False)
    ...
    def get_all_profiles(self, rubro=None):
        ...
        user_serializers = CustomerUserProfileserializersLow(profile, many=True)
