...
from apps.profile.models import CustomerUserEmail, EmailDto
from apps.profile.services import Contactservices


class CustomerUserEmailserializers(serializers.ModelSerializer):
    class Meta:
...
class CustomerUserEmailViewSet(APIView):
    def get(self, request, pk=None):
        ...
        try:
            response = contact_services.get_email(pk, request.user.id)
        except Exception as e:
            return Response({"succes": False}, status=status.HTTP_404_NOT_FOUND)

        customer_email_serializers = CustomerUserEmailserializers(response, many=False)
    ...
    def post(self, request):
        ...
        serializers = CustomerUserEmailserializers(data=request.data)
        ...
        try:
            response = contact_services.create_or_update_email(dto)
        except Exception as e:
