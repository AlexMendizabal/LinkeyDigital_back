...
from apps.profile.models import CustomerUserSocialMedia, SocialMedia, SocialMediaDto
from rest_framework.response import Response
from rest_framework import status

from apps.profile.services import SocialMediaservices
from apps.profile.views import SocialMediaserializers


class CustomerUserSocialMediaserializers(serializers.ModelSerializer):
    ...
    social_media = SocialMediaserializers(many=False, read_only=True)
...
class CustomerUserSocialMediaViewSet(APIView):
    def get(self, request, pk=None):
        ...
        try:
            response = social_media_services.get_social_media(pk, request.user.id)
        except Exception as e:
            return Response({"succes": False}, status=status.HTTP_404_NOT_FOUND)

        if pk:
            customer_social_media_serializers = CustomerUserSocialMediaserializers(response, many=False)
        else:
            customer_social_media_serializers = CustomerUserSocialMediaserializers(response, many=True)
    ...
    def post(self, request):
        ...
        serializers = CustomerUserSocialMediaserializers(data=request.data)
        ...
        try:
            response = social_media_services.create_or_update_social_media(dto)
        except Exception as e:
