...
from apps.profile.views import CustomerUserProfileserializers, \
    CustomerUserCustomSocialMediaserializers, customerUserUtilities
...
class PublicCustomerUserViewSet(APIView):
    ...
    def get(self, request, public_id):
        ...
        if not customer_user.has_access_to_protected_views():
            return Response({"succes": False, "message": "El usuario no tiene licencia"}, status=status.HTTP_503_services_UNAVAILABLE)

        customer_user_admin = utilities.getAdminLicencia(customer_user)
        customer_user_public_services = PublicCustomerUserservices()

        customer_profile_serializers = utilities.get_profile(customer_user_public_services, customer_user)
        ...
        if customer_user_admin:
            customer_profile_serializers_admin = utilities.get_profile(customer_user_public_services,
                                                                       customer_user_admin)
            customer_custom_social_media_serializers_admin = utilities.get_custom_social_media(
                customer_user_public_services, customer_user_admin, True)
...
class CustomUserUtilities():
    ...
    def get_profile(self, customer_user_public_services, customer_user):

        if self._cached_profile is not None:
            return self._cached_profile

        try:
            response = customer_user_public_services.get_profile(None, customer_user.pk)
    ...
    def get_custom_social_media(self, customer_user_public_services, customer_user, trues = False):
        try:
            if trues:
                response = customer_user_public_services.get_custom_social_media_only_true(None, customer_user.pk)
            else:
                response = customer_user_public_services.get_custom_social_media(None, customer_user.pk)
        except Exception as e:
            return Response({"succes": False}, status=status.HTTP_404_NOT_FOUND)

        customer_social_media_serializers = CustomerUserCustomSocialMediaserializers(response, many=True)
