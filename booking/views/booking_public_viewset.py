...
# Validaciones del codigo:
        ...
        booking = booking_service.get_booking_perday(customer_user=cu, specific_date=date_time_str)
        if booking.exists():
            ...
            return Response({"status": "error", "message": "La fecha no es valida, parece que hay otra reserva con la misma fecha"}, status=status.HTTP_409_CONFLICT)
